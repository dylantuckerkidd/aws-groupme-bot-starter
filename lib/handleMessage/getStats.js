...
const getStats = async function (messageText) {
    ...
    if (skill === undefined) {
        ...
        if (skill.level > 99) {
            ...
            let stringPercent = JSON.stringify(percentage)
            let formattedCalculatedXp = await formatNumber(calculatedXp)
            let text = `Pesko's ${trimmedText} level is ${skillCalculationLevel} with ${formattedXp} experience. He is ${stringPercent}% to his next level with ${formattedCalculatedXp} experience left.`
            console.log(text)
            return text
        }
    }
