...
const handleGeneralMessage = async function (messageText) {
    if (messageText === '@jamflex') {
        return `List of current commands: @jamflex, @chart, @latest, @lowest, @highest, @get [overall, attack, defence, strength, hitpoints, ranged, prayer, magic, cooking, woodcutting, fletching, fishing, firemaking, crafting, smithing, mining, herblore, agility, stickman, thieving, slayer, farming, runecrafting, hunter, construction]`
    }
    else if (messageText === '@chart') {
        return `https://i.imgur.com/UF9e3in.png`
    }

    else if (messageText === '@maple') {
        return `https://i.imgur.com/NzoZ4N6.png`
    }
    ...
    // }
        ...
        return 'https://i.imgur.com/OeBjDO1.gif'
    }
    else if (messageText.includes('spooky') || messageText.includes('Spooky')) {
        return 'https://i.imgur.com/4187ft8.gif'
    }
    else if (messageText.includes('fine') || messageText.includes('Fine')) {
        return 'Just Fine'
    }
    else if (messageText.includes('Lol') || messageText.includes('lol') || messageText.includes('haha') || messageText.includes('Haha')) {
        return 'Ha ha :-)'
    }
    else if (messageText.includes('Oof') || messageText.includes('oof')) {
        return 'https://i.imgur.com/jYOblHd.png'
    }
    else {
        return ''
    }
