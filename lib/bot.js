...
const dadBot = require('./handleMessage/dadBot')
...
class Bot {
    ...
    static async checkMessage(message) {
        ...
        else if (messageText.includes(`i'm`) || messageText.includes(`I'm`) || messageText.includes(`im`) || messageText.includes(`Im`)) {
            let text = await dadBot(messageText)
            return `${text}`
        }
        else {
